...
export const Ticket: React.FC<TicketProps> = ({ type, data, settings, onClose }) => {
    ...
    return (
        <div className="fixed inset-0 bg-slate-900/90 backdrop-blur-sm z-[999] flex items-center justify-center p-2 md:p-4 animate-fade-in">
            <div className="bg-white rounded-3xl md:rounded-[2.5rem] shadow-2xl w-full max-w-lg h-[95vh] md:h-[92vh] flex flex-col overflow-hidden animate-fade-in-up border border-white/20">
                <div className="p-4 md:p-6 bg-slate-900 text-white flex justify-between items-center shrink-0">
                    <div className="flex items-center gap-2 md:gap-3">
                        ...
                        <div>
                            <span className="font-black text-base md:text-lg block leading-none">Comprobante</span>
                            <span className="text-[8px] md:text-[10px] font-bold text-slate-400 uppercase tracking-widest">Digital & Térmico</span>
                        </div>
                    </div>
                    <button onClick={onClose} className="hover:bg-white/10 p-2 md:p-3 rounded-full transition-colors"><X className="w-5 h-5 md:w-6 md:h-6"/></button>
                </div>
                
                <div className="flex-1 overflow-y-auto p-4 md:p-8 bg-slate-200 flex justify-center custom-scrollbar">
                    <div 
                        ref={printRef} 
                        className="bg-white w-full max-w-[300px] p-4 md:p-6 shadow-2xl text-[11px] md:text-[12px] font-mono leading-tight text-black flex flex-col h-fit mb-8 md:mb-12 border-b-8 border-slate-300/30"
                    >
                        <div className="text-center mb-4 md:mb-5">
                            <h2 className="text-sm md:text-base font-black uppercase tracking-tight leading-none mb-1">{settings.name}</h2>
                            <p className="text-[9px] md:text-[10px] uppercase leading-tight mb-1 opacity-80">{settings.address}</p>
                            <p className="text-[9px] md:text-[10px] uppercase leading-tight font-bold">RUC: 10456789123</p>
                            <p className="text-[9px] md:text-[10px] uppercase leading-tight">Tel: {settings.phone}</p>
                            
                            <div className="mt-3 md:mt-4 border-2 border-black py-1 md:py-1.5 px-2 md:px-3 inline-block">
                                <p className="text-[10px] md:text-[11px] font-black uppercase">
                                    {new Date(type === 'SALE' ? (data as Transaction).date : (data.shift as CashShift).endTime!).toLocaleString('es-PE')}
                                </p>
                            </div>
                        </div>

                        <hr className="border-dashed border-black my-3 md:my-4"/>
                        
                        {type === 'SALE' ? (
                            <>
                                <div className="flex justify-between font-black text-[9px] md:text-[10px] mb-2 px-1">
                                    <span className="flex-1">DESCRIPCIÓN</span>
                                    <span className="w-16 md:w-20 text-right">TOTAL</span>
                                </div>
                                    ...
                                    {(data as Transaction).items.map((item, idx) => (
                                        <div key={idx} className="flex flex-col">
                                            <div className="flex justify-between items-start gap-2">
                                                <span className="flex-1 font-bold uppercase text-[11px] md:text-[12px]">{item.name}</span>
                                                <span className="shrink-0 font-black text-xs md:text-sm">{settings.currency}{(item.quantity * (item.price || 0)).toFixed(2)}</span>
                                            </div>
                                            <div className="flex justify-between text-[9px] md:text-[10px] font-medium italic opacity-70">
                                ...
                                <div className="space-y-1 md:space-y-1.5 px-1">
                                    ...
                                    {(Number(data.discount) || 0) > 0 && (
                                        <div className="flex justify-between font-bold text-[10px] md:text-[11px]">
                                            <span>DESCUENTO</span>
                                            <span>-{settings.currency}{(Number(data.discount) || 0).toFixed(2)}</span>
                                        </div>
                                    )}
                                    <div className="border-t border-black pt-1.5 md:pt-2 mt-1.5 md:mt-2"></div>
                                    <div className="flex justify-between text-base md:text-lg font-black tracking-tighter">
                                ...
                                <div className="bg-slate-50 p-2 rounded border border-black/10">
                                    <p className="font-black text-[9px] md:text-[10px] mb-1.5 md:mb-2 uppercase tracking-widest border-b border-black/10 pb-1">Pagado con:</p>
                ...
                <div className="p-4 md:p-8 bg-white border-t border-slate-100 flex flex-col md:flex-row gap-2 md:gap-3 shrink-0 shadow-[0_-10px_30px_rgba(0,0,0,0.05)]">
                    <button 
                        onClick={handleDownloadPDF} 
                        disabled={isGeneratingPdf}
                        className="flex-1 py-4 md:py-5 bg-indigo-50 text-indigo-700 rounded-xl md:rounded-2xl font-black hover:bg-indigo-100 transition-all flex items-center justify-center gap-2 md:gap-3 uppercase tracking-widest text-[10px] md:text-[11px] border-2 border-indigo-100 disabled:opacity-50"
                    >
                        {isGeneratingPdf ? <Loader2 className="w-4 h-4 md:w-5 md:h-5 animate-spin"/> : <Download className="w-4 h-4 md:w-5 md:h-5"/>}
                        Descargar PDF
                    </button>
                    <button onClick={handlePrint} className="flex-[1.5] py-4 md:py-5 bg-slate-900 text-white rounded-xl md:rounded-2xl font-black hover:bg-black transition-all flex items-center justify-center gap-2 md:gap-3 uppercase tracking-widest text-[11px] md:text-[12px] shadow-2xl shadow-slate-200">
                        <Printer className="w-5 h-5 md:w-6 md:h-6 text-brand"/> Imprimir Térmico
                    </button>
                </div>
            </div>
        </div>
